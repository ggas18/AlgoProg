...
def indexDistMin(listDist, listVisit):
    ...
    #
    # ENTREE: listDist la liste des distances entre la source et le sommet a la
    #                  la source
    #         listVisit liste de boolean avec listVisit[i]=True si le sommet i a
    #                   ete visite et nous connaissons la distance minimale de
    #                   la source jusqu'a ce sommet.
    #
    # SORTIE: index de l'indice du sommet de distance minimale
...
if __name__=="__main__":
    ...
    if len(sys.argv)==2:
        script,opt=sys.argv
    if(opt=="exo1"):
       print("Exercice gymnase")
        # on cree une liste d'evenements
       L=[Evenement(0,6), Evenement(1,4),Evenement(2,13),Evenement(3,5),
          Evenement(3,8),Evenement(5,7), Evenement(5,9),Evenement(6,10),
          Evenement(8,11),Evenement(9,12),Evenement(12,14)]
       ...
       for i in range(len(t)):
        ...
        A=[[0,900,4,inf,inf],
           ...
           [inf,inf,12,3,0]]
        arbCouvrant=ACM(A)
        for k in range(len(arbCouvrant)):
            print(arbCouvrant[k])
